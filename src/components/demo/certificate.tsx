import {
  ...
  useEffect(() => {
     const  getData = async () => {
      try {
        const { data } = await axios.get(`http://localhost:8803/reporting-app/certificates/getAll?fromDate=${startDate}&toDate=${endDate}&intermediate=${intermediate}`)
      ...
      <div>
        <NavBar startDate={startDate}  endDate={endDate} intermediate={intermediate} />
           <div ref={conponentPDF} style={{width: '100%', marginTop: '50px'}}>
            <h1 className="text-center text-gray-600 text-3xl font-bold @apply uppercase mb-9">Certificate </h1>
        ...
        <TableCaption>A list of your certificate</TableCaption>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {valueTable?.map((value, idx) => (
            <TableRow key={idx}>
              ...
              <TableCell>{value.STATUT}</TableCell>
